...
Text embedding is not yet supported in the Dart Google GenAI plugin.
:::
...
:::note[Feature unavailable for Dart]
The Gemini Files API is not yet available in the Dart SDK.
:::
...
:::note[Feature unavailable for Dart]
Video generation via Veo is not yet fully documented/supported in the Dart examples.
:::
...
import 'package:genkit/genkit.dart';
import 'package:genkit_google_genai/genkit_google_genai.dart';
// Note: You'll need a way to save the file, e.g., 'dart:io'
...
if (media != null) {
  // Save media.url (data URI) to specific file
  // String base64Data = media.url.split(',')[1];
